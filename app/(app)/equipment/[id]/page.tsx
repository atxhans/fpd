...
export default async function EquipmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Persist health score so the listing page always shows the same value
  await supabase.from('equipment').update({ health_score: healthBreakdown.total, health_score_at: new Date().toISOString() }).eq('id', id)
